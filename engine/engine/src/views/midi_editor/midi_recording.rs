...
fn do_midi_recorder_animation_tick(ctx_ptr: *mut MIDIRecordingContext, cur_time: f64) {
    let recording_ctx = unsafe { &mut *ctx_ptr };

    let total_seconds_recorded = cur_time - recording_ctx.start_time_seconds;
    let total_beats_recorded = time_to_beats(recording_ctx.bpm, total_seconds_recorded);
    ...
    MidiEditorGridRenderer::set_cursor_pos(recording_ctx.grid_state.cursor_dom_id, cursor_pos_px);
    ...
    for entry_opt in &recording_ctx.active_voices {
        if let Some(entry) = entry_opt {
            ...
            let note_length_beats = time_to_beats(recording_ctx.bpm, note_length_seconds);
...
pub fn start_recording_midi(
    ...
    let recording_ctx = box MIDIRecordingContext::new(state, grid_state, cur_time, bpm);
...
pub fn midi_editor_record_note_down(
    ...
    let recording_ctx = unsafe { &mut *recording_ctx_ptr };
    ...
    for (i, slot) in recording_ctx.active_voices.iter().enumerate() {
        match slot {
            Some(voice) if note_id == voice.note_id => {
                warn!(
                    "MIDI recorder registered key down for note id {} but one is already down for \
                     that note id",
                    note_id
                );
                return;
    ...
    if let Some(first_empty_ix) = first_empty_ix {
        ...
        let start_beat = time_to_beats(
            recording_ctx.bpm,
            cur_time - recording_ctx.start_time_seconds,
...
pub fn midi_editor_record_note_up(
    ...
    let voice_entry_ix = match recording_ctx
        ...
        None => {
            warn!(
                "No playing note with note id {} found in MIDI recorder; ignoring note up event...",
                note_id
            );
            return;
        },
    ...
    let note_start_beat = time_to_beats(
        recording_ctx.bpm,
        entry.playing_start_time_seconds - recording_ctx.start_time_seconds,
    ) + recording_ctx.initial_cursor_pos_beats;
    let note_length_seconds = cur_time - entry.playing_start_time_seconds;
    let note_length_beats = time_to_beats(recording_ctx.bpm, note_length_seconds);
