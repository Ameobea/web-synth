...
impl VocoderCtx {
    ...
    fn build_filter_bands() -> Box<[VocoderBand; BAND_COUNT]> {
        ...
        for band_ix in 0..BAND_COUNT {
            ...
            for filter_ix in 0..FILTERS_PER_BAND {
                ...
                unsafe {
                    let cutoff_freq =
                        FILTER_PARAMS_BUF[(FILTERS_PER_BAND * band_ix + filter_ix) * 2];
    ...
    pub fn new() -> Self {
        ...
        for band_ix in 0..BAND_COUNT {
            let lowpass_freq = unsafe {
                FILTER_PARAMS_BUF[(FILTERS_PER_BAND * band_ix + FILTERS_PER_BAND / 2 - 1) * 2]
            };
            let highpass_freq = unsafe {
                FILTER_PARAMS_BUF[(FILTERS_PER_BAND * band_ix + FILTERS_PER_BAND / 2) * 2]
            };
