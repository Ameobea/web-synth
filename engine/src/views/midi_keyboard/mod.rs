...
impl MIDIKeyboard {
    ...
    pub fn get_state_key(&self) -> String {
        format!("MIDIKeyboard_{}", self.uuid)
    }
}

impl ViewContext for MIDIKeyboard {
    fn init(&mut self) {
        js::init_midi_keyboard(&self.get_state_key());
    }
    ...
    fn hide(&mut self) {
        js::hide_midi_keyboard(&self.get_state_key());
    }

    fn unhide(&mut self) {
        js::unhide_midi_keyboard(&self.get_state_key());
    }
