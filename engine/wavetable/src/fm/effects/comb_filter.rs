...
impl Effect for CombFilter {
    ...
    fn apply(&mut self, rendered_params: &[f32], _base_frequency: f32, input_sample: f32) -> f32 {
        ...
        let feedback_delay_samples =
            dsp::clamp(0., MAX_DELAY_SAMPLES as f32 - 1., rendered_params[1]);
        ...
        let with_feedback = with_feedforward
            + feedback_gain * self.feedback_buffer.read_interpolated(-delay_samples);
    ...
    fn apply_all(
        ...
        for sample_ix in 0..samples.len() {
            let delay_samples =
                dsp::clamp(0., MAX_DELAY_SAMPLES as f32 - 1., delay_samples[sample_ix]);
